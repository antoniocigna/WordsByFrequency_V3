...
//--------------------------------------------------------
func g09_bind_go_write_lang_dictionary( langAndVoiceName  string) { 
		return
		/**
		outFileName		:= FOLDER_INPUT_OUTPUT + string(os.PathSeparator) + FILE_inputLanguage;  		
...
//-----------------------------------------------------------
func g09_bind_go_write_word_dictionary( listGoWords string) { 
...
func split_one_word_dict_row( row1 string ) (string, int, []string, []string) {
	...
	ixFr, err := strconv.Atoi( field[1] )
	if err != nil { return "",	-1,	lemmaLis, tranLis }                           //error
	 
	return field[0],ixFr, lemmaLis, tranLis     // eg. return einem; 14, [ein , einem,   einer], [uno , uno,     uno] 
...
//---------------------------------------------------------
//-------------------------------------
func g09_bind_go_write_row_dictionary( listGoRows string) {
		/**
		2;Primo capitolo
...
func g09_2_bind_go_write_new_row_dictionary( listGoRows string, js_function string) {
	...
	for _,row1dict := range  strings.Split(listGoRows,"\n") {  
		if row1dict == "" { continue }	
		lineZ :=  strings.TrimSpace(row1dict) + "||||"		
	...
	fmt.Println("bind_go_write_new_row_dictionary ()  esegue  writeTextRowSlice () ")
...
func g09_bind_go_passToJs_write_WordsToLearn(js_function string)  {
	...
	numOut:=0
	for _,xWordAlpha := range uniqueWordByAlpha { 	
		//if ( (xWordAlpha.uKnow_yes_ctr < 1 ) && (xWordAlpha.uKnow_no_ctr < 1 ))  { continue }
		//lines = append( lines,   fmt.Sprintf( "%s|%d|%d",  xWordAlpha.uWord2, xWordAlpha.uKnow_yes_ctr,  xWordAlpha.uKnow_no_ctr) ) 
		lines = append( lines,  xWordAlpha.uWord2 + "|" + xWordAlpha.uLearnedYN  ) 
		numOut++	
	}	

	fmt.Println("write_WordsToLearn   num=", numOut); 
	if (numOut > 0) { 	
