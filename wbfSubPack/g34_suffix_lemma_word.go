...
func g34_lookForInverse(invTarg string, inverseSlice []inverseStruct, lastIx int) (int, int) {
...
func g34_load_direct_and_inverse_lemma()  {	
	...
	*/
	for lemIndex, dirLem := range lemmaSlice {
		if dirLem.leNumWords == 0 { continue }		// nessuna word del testo ha questo lemma 	
	...
	provaDirectLemma("gehen",true,  100)
	provaDirectLemma("geh"  ,false, 100)
	**/
...
func g34_getListInverseLemmaIndex(dirLemmaTarg string, sw_oneLemma bool, maxNum int) []int { 
	...
	fromIx, toIx:= g34_lookForInverse(invLemmaTarg, inverseLemmaSlice,0)
	...
	if fromIx < 0 { return listInverseLemmaIndex }
...
func g34_getListDirectLemmaIndex(dirLemmaTarg string, sw_oneLemma bool, maxNum int) []int { 
	...
	fromIx, toIx:= g34_lookForInverse(invLemmaTarg, directLemmaSlice,0)
	...
	lenCk  := 0
	num:=0
	for j:=fromIx; j < len(directLemmaSlice); j++ {		
...
func g34_getListInverseWordIndex(dirWordTarg string, maxNum int) []int { 
	...
	fromIx, toIx:= g34_lookForInverse(invWordTarg, inverseWordSlice,0)
	if toIx < fromIx { fromIx = toIx}
	...
	if fromIx < 0 { return listInverseWordIndex }
...
func provaInverseWord( finalWord string, maxNum int) {
	...
	listInverseWordIndex := g34_getListInverseWordIndex( finalWord, maxNum) 
