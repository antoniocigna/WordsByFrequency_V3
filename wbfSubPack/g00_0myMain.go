...
func TOGLIread_dictLang_file( path0 string, inpFile0 string) {
	
	path1, inpFile1 := getLemmaPathAndFile(inpFile0)
	if path1 == "" { path1 = path0}
	
	bytesPerRow:= 10
    lineD := rowListFromFile( path1, inpFile1, "scelta lingua e voce", "read_dictLang_file", bytesPerRow)  
	if sw_stop { return }
...
}// end of TOGLIread_dictLang_file		
